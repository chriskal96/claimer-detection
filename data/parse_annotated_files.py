    ...
    for dir in dirs:
        if dir.endswith(".txt"):
            with zipfile.ZipFile(
                "annotation/" + dir + "/annotators.zip", "r"
            ) as zip_ref:
                zip_ref.extractall("annotation/" + dir + "/annotators")

            with open("annotation/" + dir + "/annotators/annotators.xmi", "r") as f:
                data = f.read()
            ...
            reference = Bs_data.find(
                "custom:ClaimDetectionSchema", {"Label": "Reference"}
            )
            if topic:
...
with open("annotation/annotated_dataset.json", "w") as outfile:
